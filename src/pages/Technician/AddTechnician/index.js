...
const AddTechnician = () => {
  ...
  const registerNewCar = async (car) => {
    const catFormatted = buildCar(car);
    try{
      const { data ,status } = await newCar(catFormatted);
  ...
  const saveTechnician = async (e) => {
    const technicianFormatted = buildTechnician({...e, responsibleUser: 'modrp'});
    try{
      const { status } = await newTechnician(technicianFormatted);
